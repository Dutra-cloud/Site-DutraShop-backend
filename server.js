...
app.use((req, res, next) => { console.log(`[${new Date().toISOString()}] Recebida requisição: ${req.method} ${req.url}`); next(); });

const pool = new Pool({ connectionString: process.env.DATABASE_URL, ssl: { rejectUnauthorized: false } });

// --- FUNÇÃO PARA POPULAR O BANCO DE DADOS (NOVA) ---
const seedDatabaseIfNeeded = async () => {
    try {
        // 1. Verifica se a tabela de produtos já tem algum item
        const productCount = await pool.query('SELECT COUNT(*) FROM products');
        if (parseInt(productCount.rows[0].count) > 0) {
            console.log("Banco de dados já populado. Nenhuma ação necessária.");
            return;
        }

        // 2. Se estiver vazia, insere os produtos iniciais
        ...
        for (const p of productsToInsert) {
            await pool.query(
                "INSERT INTO products (id, name, price, image, rating, reviewCount, category, stock) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)",
                [p.id, p.name, p.price, p.image, p.rating, p.reviewCount, p.category, p.stock]
            );
        }
...
const createTables = async () => {
    // ... (função createTables continua a mesma)
    ...
    try {
        ...
        // Chama a função para popular o banco de dados APÓS criar as tabelas
...
// Colando as rotas completas para garantir
