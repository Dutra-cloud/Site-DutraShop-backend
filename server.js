...
// O Render vai nos dar uma URL de conexão e colocá-la em process.env.DATABASE_URL
const pool = new Pool({
...
// --- ROTAS DA API (AGORA USANDO A SINTAXE DO 'pg') ---
...
// Atualizar o estoque
app.patch('/api/products/update-stock', async (req, res) => {
    const client = await pool.connect();
    try {
        const { items } = req.body;
        await client.query('BEGIN');
        for (const item of items) {
            await client.query('UPDATE products SET stock = stock - $1 WHERE id = $2 AND stock >= $1', [item.quantity, item.id]);
        }
        await client.query('COMMIT');
        res.status(200).json({ success: 'Estoque atualizado com sucesso!' });
    } catch (err) {
...
app.post('/api/register', async (req, res) => {
    try {
        ...
        console.error(err);
        res.status(500).json({ error: 'Email já pode estar em uso ou erro no servidor' });
    }
});
...
app.post('/api/login', async (req, res) => {
    try {
        ...
        if (!user) {
            return res.status(404).json({ error: 'Usuário не encontrado' });
        }
...
app.listen(PORT, () => {
    ...
    createTables(); // Cria as tabelas ao iniciar o servidor
});